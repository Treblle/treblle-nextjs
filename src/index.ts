...
import { TREBLLE_ENDPOINTS } from "./constants";
...
class Treblle {
    ...
    constructor(options: TreblleOptions) {
        // Use environment variables as fallbacks similar to treblle-express
        const sdkToken = options.sdkToken || process.env.TREBLLE_SDK_TOKEN;
        ...
        // Initialize path filters - support both blocklistPaths and excludePaths
        this.excludePaths = options.blocklistPaths || options.excludePaths || [];
        ...
        if (this.debug) {
            console.log(
                `[Treblle SDK] Initialized in ${getCurrentEnvironment()} environment. SDK ${this.enabled ? "enabled" : "disabled"}.`
            );
        }
    }
    ...
    middleware() {
        ...
        return (req: any, res: any, next: any) => {
            ...
            res.end = function (chunk: any, _encoding?: string) {
                ...
                const contentDisposition = res.getHeader
                    ? res.getHeader("content-disposition") || ""
                    : "";

                // Check if response is a file or non-JSON content
                const isFile =
                    contentDisposition.toString().includes("attachment") ||
                ...
                if (
                    ...
                    (typeof responseBody === "object" &&
                        responseBody &&
                        "__type" in responseBody &&
                        responseBody.__type === "binary")
                ) {
                    ...
                    data: {
                        ...
                        request: {
                            ...
                            headers: maskSensitiveData(
                                req.headers,
                                self.options.additionalMaskedFields
                            ),
    ...
    private _sendPayload(payload: any): void {
        try {
            ...
            const endpoint =
                TREBLLE_ENDPOINTS[Math.floor(Math.random() * TREBLLE_ENDPOINTS.length)];
                    ...
                    res.on("end", () => {
                        if (responseData) {
                            try {
                                ...
                                console.log(
                                    `[Treblle SDK] Response from Treblle API: ${JSON.stringify(parsedResponse)}`
                                );
                            } catch (e) {
                                console.log(
                                    `[Treblle SDK] Raw response from Treblle API: ${responseData}`
                                );
                            }
                        } else {
    ...
    private _shouldExcludePath(path: string): boolean {
        // If the pattern is a RegExp, test against it directly
        if (this.excludePaths instanceof RegExp) {
            return this.excludePaths.test(path);
        }

        // Otherwise, treat as array and check each pattern
        for (const pattern of this.excludePaths) {
            if (pattern instanceof RegExp) {
                ...
                // For string patterns, check if path starts with the pattern prefixed with "/"
                // This matches treblle-express behavior
                if (path.startsWith(`/${pattern}`)) {
                    return true;
                }

                // Also check exact match with or without leading slash
