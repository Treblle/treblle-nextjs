...
function getTreblleInstance(options: TreblleOptions): Treblle {
    ...
    if (options.debug) {
        console.log(
            `[Treblle SDK] NestJS: Getting Treblle instance with hash ${hash.substring(0, 20)}...`
        );
...
function getDefaultInstance(): Treblle {
    if (!defaultInstance) {
        throw new Error(
            "No Treblle instance found. Create middleware with options first or provide options directly."
        );
    }
...
export class TreblleModule {
    ...
    static createMiddleware(
        ...
        return (req: Request, res: Response, next: NextFunction) => {
            if (options.debug) {
                console.log(
                    `[Treblle SDK] NestJS: Function middleware executing for ${req.method} ${req.url}`
                );
            }
            middleware(req, res, next);
        };
